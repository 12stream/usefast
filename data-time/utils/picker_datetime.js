...
pickerDatetime.prototype.setPicker = function(pickerName) {
  ...
  var obj = new Object();
  obj[pickerName] = pickerPreYear + '-' + pickerPreMonth + '-' + (pickerPreDay < 10 ? '0' + pickerPreDay : pickerPreDay) + ' ' + addDatetimeZero(pickerPreHour) + ':' + addDatetimeZero(pickerPreMinute) + ':' + addDatetimeZero(pickerpreSecond);
  setTimeout(()=>{
  ...
  for (let y = pickerNowYear-1; y <= pickerNowYear+1; y++) {
    for (let m = 1 ; m <= 12; m++) {
      ...
      for (let d = 1; d <= m_days; d++){
        ...
        this.pickerDateValueArr.push(y+'-'+m+'-'+(d<10?'0'+d:d));
      } 
    }
  }
  //获取小时和分钟的数组，设置默认值
  ...
  this.page.bindChange = function(e) {
    var pickerPreDate = new Date();
    var pickerpreSecond = pickerPreDate.getSeconds() < 10 ? '0' + pickerPreDate.getSeconds() : pickerPreDate.getSeconds();
    var val = e.detail.value
    var newObj = {};
    newObj[pickerName] = _this.pickerDateValueArr[val[0]] + ' ' + _this.pickerHourTextArr[val[1]] + ':' + _this.pickerMinuteTextArr[val[2]] + ':' + pickerpreSecond;
    this.setData(newObj);
  };
